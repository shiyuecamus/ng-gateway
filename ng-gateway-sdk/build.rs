...
fn main() {
    ...
    let api_version = env::var("NG_SDK_API_VERSION")
        .unwrap_or(env::var("CARGO_PKG_VERSION_MAJOR").unwrap_or("1".to_string()));
    println!("cargo:rustc-env=NG_SDK_API_VERSION={}", api_version);

    // SDK crate version (SemVer)
    let pkg_version = env::var("CARGO_PKG_VERSION").unwrap_or("0.0.0".to_string());
