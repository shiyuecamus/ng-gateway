...
async fn validate_role(role: &RoleActiveModel, operation: Operation) -> WebResult<()> {
    let duplicate = match operation {
        Operation::Create => {
            RoleRepository::exists_by_code(role.code.to_owned().take().unwrap().as_str()).await?
        }
        Operation::Write => {
            RoleRepository::exists_by_code_exclude_id(
...
async fn validate_user(user: &UserActiveModel, operation: Operation) -> WebResult<()> {
    let duplicate = match operation {
        Operation::Create => {
            UserRepository::exists_by_username_email_phone(
        ...
        Operation::Write => {
            UserRepository::exists_by_username_email_phone_exclude_id(
...
async fn validate_driver(driver: &DriverActiveModel, operation: Operation) -> WebResult<()> {
    let duplicate = match operation {
        Operation::Create => {
            DriverRepository::exists_by_type_and_version(
        ...
        Operation::Write => {
            DriverRepository::exists_by_type_and_version_exclude_id(
...
async fn validate_plugin(plugin: &PluginActiveModel, operation: Operation) -> WebResult<()> {
    let duplicate = match operation {
        Operation::Create => {
            PluginRepository::exists_by_type_and_version(
        ...
        Operation::Write => {
            PluginRepository::exists_by_type_and_version_exclude_id(
...
async fn validate_device(device: &DeviceActiveModel, operation: Operation) -> WebResult<()> {
    let duplicate = match operation {
        Operation::Create => {
            DeviceRepository::exists_by_name(device.device_name.to_owned().take().unwrap()).await?
        }
        Operation::Write => {
            DeviceRepository::exists_by_name_exclude_id(
...
async fn validate_point(point: &PointActiveModel, operation: Operation) -> WebResult<()> {
    let duplicate = match operation {
        Operation::Create => {
            PointRepository::exists_by_device_and_key(
        ...
        Operation::Write => {
            PointRepository::exists_by_device_and_key_exclude_id(
...
async fn validate_action(action: &ActionActiveModel, operation: Operation) -> WebResult<()> {
    let duplicate = match operation {
        Operation::Create => {
            ActionRepository::exists_by_device_and_command(
        ...
        Operation::Write => {
            ActionRepository::exists_by_device_and_command_exclude_id(
