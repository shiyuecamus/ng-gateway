...
async fn wait_connected(driver: &Dl645Driver) {
    ...
    .await
    .expect("timeout waiting for DL/T 645-1997 driver to connect, please check simulator");
}
    ...
    F: FnOnce(i32) -> Dl645Action,
{
    ...
    driver
        .start()
        .await
        .unwrap_or_else(|e| panic!("Dl645Driver::start (1997) failed: {e}"));
    ...
    match res {
        Ok(v) => {
            tracing::info!(
                ...
                action = %label,
                result = ?v,
                "DLT645-1997 execute_action succeeded"
            );
        }
        Err(e) => {
            tracing::warn!(
                ...
                action = %label,
                error = %e,
                "DLT645-1997 execute_action returned error"
            );
        }
    }
    ...
    if let Err(e) = stop_res {
        tracing::warn!(
            ...
            action = %label,
            error = %e,
            "Dl645Driver::stop (1997, single action test) returned error"
        );
    }
}

/// Manual integration test: DL/T 645-1997 `execute_action` (broadcast time sync).
#[tokio::test]
#[ignore]
async fn broadcast_time_sync() {
    ...
    let dt = Utc
        .with_ymd_and_hms(2025, 12, 10, 23, 46, 25)
        .single()
...
#[ignore]
async fn write_data() {
    let params = json!({ "energy": 123456.88 });
...
async fn write_address() {
    let params = json!({ "newAddress": "123456789012" });
