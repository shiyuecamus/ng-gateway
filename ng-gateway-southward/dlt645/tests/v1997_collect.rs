...
///
/// This test expects a DL/T 645-1997 slave simulator to be running locally on
...
/// manually stop and restart the simulator to observe reconnect behavior.
#[tokio::test]
#[ignore]
async fn reconnect_and_collect() {
    ...
    init_tracing();
    // Adjust these values to match your local DL/T 645-1997 slave simulator.
    let host = "127.0.0.1";
    let port: u16 = 5679;
    let device_address = "000000000000";
    // DI used for the test point; update it according to the simulator setup.
    let point_dis = ["9010", "9011"];
    ...
    let driver = match Dl645Driver::with_context(ctx) {
        Ok(d) => d,
        Err(e) => {
            panic!("failed to create Dl645Driver (1997): {e}");
        }
    };

    if let Err(e) = driver.start().await {
        panic!("Dl645Driver::start (1997) failed: {e}");
    }
        ...
        match res {
            Ok(values) => {
                tracing::info!(
                    ...
                    count = values.len(),
                    data = ?values,
                    "DLT645-1997 collect_data succeeded"
                );
            }
            Err(e) => {
                tracing::warn!(
                    ...
                    iteration,
                    error = %e,
                    "DLT645-1997 collect_data error"
                );
            }
        }
    ...
    if let Err(e) = stop_res {
        tracing::warn!(
            target: "dlt645::test",
            error = %e,
            "Dl645Driver::stop (1997) returned error"
        );
    }
}
