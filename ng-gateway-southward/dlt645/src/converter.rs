//! Model converter for the DL/T 645 driver.
//!
...
/// Default model converter for the DL/T 645 driver.
#[derive(Debug, Clone, Default)]
pub struct Dl645Converter;

impl SouthwardModelConverter for Dl645Converter {
    ...
    fn convert_runtime_device(&self, device: DeviceModel) -> DriverResult<Arc<dyn RuntimeDevice>> {
        ...
        let address = encode_address_from_str(&address).map_err(|e| {
            DriverError::ConfigurationError(format!("Invalid DL/T 645 address {address}: {e}"))
        })?;
                ...
                let di = driver_config
                    ...
                    .map(|v| {
                        parse_di_str(v).ok_or(DriverError::ConfigurationError(format!(
                            "Invalid DI for DL/T 645 parameter: {v}"
                        )))
                    })
                    .transpose()?;
                let decimals = driver_config
