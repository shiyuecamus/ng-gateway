...
///
/// This test expects a CJ/T 188-2018 slave simulator.
#[tokio::test]
#[ignore]
async fn reconnect_and_collect() {
    ...
    if wait_res.is_err() {
        panic!("timeout waiting for CJ/T 188-2018 driver to connect, please check simulator");
    }
        ...
        match res {
            Ok(values) => {
                tracing::info!(
                    ...
                    count = values.len(),
                    data = ?values,
                    "CJT188-2018 collect_data succeeded"
                );
            }
            Err(e) => {
                tracing::warn!(
                    ...
                    iteration,
                    error = %e,
                    "CJT188-2018 collect_data error"
                );
            }
        }
    ...
    if let Err(e) = stop_res {
        tracing::warn!(
            target: "cjt188::test",
            error = %e,
            "Cjt188Driver::stop (2018) returned error"
        );
    }
}
