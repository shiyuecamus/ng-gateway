//! Model converter for the CJ/T 188 driver.
//!
...
use super::types::{Cjt188Channel, Cjt188Device, Cjt188Point, MeterType};
use ng_gateway_sdk::{
...
/// Default model converter for the CJ/T 188 driver.
#[derive(Debug, Clone, Default)]
pub struct Cjt188Converter;

impl SouthwardModelConverter for Cjt188Converter {
    ...
    fn convert_runtime_device(&self, device: DeviceModel) -> DriverResult<Arc<dyn RuntimeDevice>> {
        ...
        let meter_type_code = driver_config
            .get("meterType")
            .or(driver_config.get("meter_type"))
            .and_then(|v| v.as_u64())
    ...
    fn convert_runtime_action(&self, _action: ActionModel) -> DriverResult<Arc<dyn RuntimeAction>> {
        Err(DriverError::ConfigurationError(
            "CJ/T 188 driver does not support actions (downlink is not implemented)".to_string(),
        ))
    }
}
