...
#[test]
fn secure_channel_none_security_generates_null_local_nonce() {
    let mut secure_channel = SecureChannel::new_no_certificate_store();
    secure_channel.set_security_policy(SecurityPolicy::None);
    secure_channel.set_security_mode(MessageSecurityMode::None);

    secure_channel.create_random_nonce();

    assert!(secure_channel.local_nonce().is_empty());
    assert_eq!(
...
#[test]
fn secure_channel_secure_policy_generates_policy_sized_local_nonce() {
    ...
    assert_eq!(
        secure_channel.local_nonce().len(),
        security_policy.secure_channel_nonce_length()
    );
