...
impl SecureChannel {
    ...
    /// Creates a nonce for the connection.
    ///
    /// OPC UA Part 6 states that when `SecurityMode=None`, OpenSecureChannel
    /// messages are not signed or encrypted, nonces are ignored, and the nonce
    /// fields should be set to null. Keep the in-memory nonce empty in that
    /// case so `local_nonce_as_byte_string()` serializes a protocol-compliant
    /// null `ByteString` for strict clients such as KepServerEX.
