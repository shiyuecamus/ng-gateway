...
pub type SouthwardRegistry = Arc<DashMap<i32, Arc<dyn DriverFactory + Send + Sync>>>;
...
#[derive(Clone)]
pub struct SouthwardLoader {
    registry: SouthwardRegistry,
...
impl SouthwardLoader {
    /// Create a new driver loader with the given registry.
    pub fn new(registry: SouthwardRegistry) -> Self {
        Self {
            registry,
    ...
    pub async fn load_driver_library(
        ...
        path: &Path,
        driver_id: i32,
    ) -> DriverResult<SouthwardProbeInfo> {
        tracing::info!(
...
fn extract_probe_info(library: &Library, path: &Path) -> DriverResult<SouthwardProbeInfo> {
...
pub fn probe_driver_library(path: &Path) -> DriverResult<SouthwardProbeInfo> {
    ensure_current_platform_from_path(path)?;
